...
async function loadAlgorithmModules() {
    ...
    const basePath = '../算法/';
    try {
        console.log('[loadAlgorithmModules] 开始加载算法模块，路径:', basePath);
        ...
        console.log('[loadAlgorithmModules] 算法模块加载成功');
        _algoModules = {
        ...
        console.error('[loadAlgorithmModules] 算法模块加载失败:', error);
        // 返回一个模拟的算法模块，避免页面完全崩溃
        // 但标记为降级模式，让调用者知道算法不可用
        _algoModules = {
            calculateBazi: () => { throw new Error('算法模块加载失败，无法计算八字'); },
            getCurrentLiuNian: () => ({ '流年': { ganzhi: '未知' } }),
            getYearsLiuNian: () => [],
            calculateShenSha: () => ({ shensha_by_column: {} }),
            calculateDayun: () => ({ dayun: [], xiaoyun: [], direction: '顺行' }),
            _degraded: true
        };
        console.warn('[loadAlgorithmModules] 返回降级算法模块');
        return _algoModules;
    }
}
...
async function loadRecordFromUrl() {
    // 防止重复执行（可选）
    if (window._loadRecordFromUrlRedirected) {
        console.warn('[loadRecordFromUrl] 已执行，避免重复');
        return;
    }

    try {
        console.log('[loadRecordFromUrl] 开始加载记录，完整 URL:', window.location.href);
        ...
        console.log('[loadRecordFromUrl] 当前时间:', new Date().toISOString());
        ...
        console.log('[loadRecordFromUrl] 开始初始化数据库...');
        if (!db.db) {
            console.log('[loadRecordFromUrl] 数据库未初始化，正在初始化...');
            try {
                await db.init();
                console.log('[loadRecordFromUrl] 数据库初始化成功');
            } catch (dbError) {
                console.error('[loadRecordFromUrl] 数据库初始化失败:', dbError);
                alert('数据库初始化失败: ' + dbError.message);
                // 不跳转，让用户留在当前页面
                return;
            }
        } else {
            console.log('[loadRecordFromUrl] 数据库已初始化');
        }

        console.log('[loadRecordFromUrl] 正在获取记录，ID:', recordId);
        const record = await db.getRecord(parseInt(recordId));

        console.log('[loadRecordFromUrl] 记录获取成功:', record);
        console.log('[loadRecordFromUrl] fullData:', record?.fullData);

        if (!record) {
            console.log('[loadRecordFromUrl] 记录不存在');
        ...
        console.log('[loadRecordFromUrl] 开始显示记录数据');
        currentRecord = record;
        await displayRecordData(record);
        console.log('[loadRecordFromUrl] 记录显示完成');
    } catch (error) {
        console.error('[loadRecordFromUrl] 加载记录失败:', error);
        console.error('[loadRecordFromUrl] 错误堆栈:', error.stack);
